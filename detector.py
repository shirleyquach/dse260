...
class Detector(AbstractDetector):
    ...
    def manual_configure(self, models_dirpath: str):
        ...
        for kernel in ['rbf', 'linear', 'poly', 'sigmoid', 'cosine']:
        # layer_pca_components = [10]
        # ica_components = [5]
        # for kernel in ['poly']:
            print('Generating: ', kernel)
            for ica_component in tqdm(ica_components, desc='ica'):
                for layer_pca_component in tqdm(layer_pca_components, desc='layer'):
                    for arch_pca_component in tqdm(arch_pca_components, desc='arch'):
                        for dataset_pca_component in tqdm(dataset_pca_components, desc='pca', leave=False):
                            try:
