...
l_bounds = np.asarray(
    [
        (0 - cifar.mean[0]) / cifar.std[0],
        (0 - cifar.mean[1]) / cifar.std[1],
        (0 - cifar.mean[2]) / cifar.std[2],
    ]
)
h_bounds = np.asarray(
    [
        (255 - cifar.mean[0]) / cifar.std[0],
        (255 - cifar.mean[1]) / cifar.std[1],
        (255 - cifar.mean[2]) / cifar.std[2],
    ]
)
...
def filter_stamp(n_img, trigger):
    ...
    i_image = tf.reshape(
        tf.matmul(
            tf.reshape(tf.concat([t_image, imax, imin, iavg], axis=3), (-1, 12)), tdelta
        ),
        [-1, h, w, 3],
    )
...
def test(weights_file, test_xs, result, mode="mask"):
    ...
    for i in range(len(t_images)):
        imageio.imsave(f"{tdname}/" + "{0}.png".format(i), cifar.deprocess(t_images[i]))
...
if __name__ == "__main__":
    ...
    tlabel = 0
    rdelta = pickle.load(
        open("./deltas/nin_trojan_dark_red_1_1_model_conv2d_7_135_64_0.pkl", "rb")
    )
    rmask = pickle.load(
        open("./masks/nin_trojan_dark_red_1_1_model_conv2d_7_135_64_0.pkl", "rb")
    )
    rimg = imageio.imread(
        "./imgs/nin_trojan_dark_red_1_1_model_conv2d_7_135_64_0_0.png"
    )
